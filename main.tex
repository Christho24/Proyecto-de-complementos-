...
La plataforma Finis Trabaja no nace simplemente como un requerimiento institucional, sino como una respuesta tecnológica directa a una problemática operativa que nosotros mismos y nuestros compañeros enfrentamos a diario. Actualmente, la búsqueda de prácticas profesionales o primeros empleos depende de canales informales, correos masivos y grupos de mensajería que generan pérdida de información, nula trazabilidad y desconexión con el mercado laboral. 

Nuestra solución plantea el diseño e implementación de una plataforma web multicapa orientada a centralizar, automatizar y optimizar todo el ecosistema de oportunidades laborales de la Universidad Finis Terrae. A través de una arquitectura basada en componentes, el sistema no solo funciona como un repositorio de ofertas, sino que integra de manera activa la administración de perfiles digitales (Currículum Vitae Virtual), la publicación estructurada de vacantes y, como núcleo innovador, un motor de recomendación algorítmica. Esto nos permite pasar de un modelo pasivo de búsqueda a un sistema proactivo que cruza inteligentemente la oferta y la demanda, garantizando la seguridad de la información y entregando métricas reales de empleabilidad a la universidad.

\subsection{Características de los participantes}%(benja)

Para asegurar que la arquitectura de Finis Trabaja resuelva el problema de fondo, es indispensable separar a los actores que tienen un interés estratégico y directivo en el proyecto (stakeholders) de aquellos que interactuarán operacionalmente con las interfaces de la plataforma en el día a día (usuarios).

\subsubsection{Stakeholders}%(benja)

Los stakeholders corresponden a los grupos e individuos en los que recae el impacto del proyecto a nivel institucional, académico y comercial:

<<<<<<< HEAD
\begin{itemize}
    \item \textbf{Dirección de Vinculación con el Medio:} Representada por su Director(a) y los Coordinadores de Empleabilidad. Son la contraparte principal y quienes levantan la necesidad. Su mayor interés es contar con una herramienta que les permita estandarizar las comunicaciones, dejar de depender de planillas o procesos manuales y, sobre todo, obtener métricas y KPIs consolidados sobre cuántos alumnos realmente logran insertarse en el mercado laboral gracias a la gestión de la universidad.
    
    \vspace{0.2cm}
    
    \item \textbf{Autoridades de la Universidad Finis Terrae:} Involucra a la Rectoría, Vicerrectoría Académica, Decanos y Directores de Escuela (como en nuestra propia carrera de Ingeniería Civil en Informática y Telecomunicaciones). Buscan la modernización tecnológica de los procesos universitarios y mejorar la percepción de calidad institucional, demostrando un compromiso real, medible y auditable con el futuro profesional de sus estudiantes.
    
    \vspace{0.2cm}
    
    \item \textbf{Sector Productivo (Mercado Laboral):} Abarca a Gerentes de Recursos Humanos, Analistas de Selección y Jefaturas técnicas de empresas colaboradoras. Su "dolor" actual es recibir cientos de postulaciones desestructuradas que no calzan con el perfil buscado. Se benefician al tener acceso directo a un semillero de talento ya validado académicamente, reduciendo drásticamente sus costos y tiempos de reclutamiento.
    
    \vspace{0.2cm}
    
    \item \textbf{Equipo de Desarrollo:} Compuesto por nosotros (Christopher, Benjamín, Maico y Camila). Nuestro interés técnico y profesional es aplicar nuestra formación en ingeniería para diseñar una arquitectura de software escalable, lograr que los algoritmos de compatibilidad entreguen recomendaciones de alto valor y asegurar que el sistema final cumpla con los estándares de calidad esperados.
\end{itemize}
=======
\begin{table}[H]
\centering
...
\newpage
>>>>>>> c88184529ceceb2395ad77790dd2684e059e33ea

\subsubsection{Usuarios del sistema}%(benja)

Los usuarios directos, definidos en nuestro modelo de Casos de Uso, interactuarán con módulos específicos según su nivel de privilegios y necesidades:
...
\hline
<<<<<<< HEAD
\textbf{Perfil de Usuario} & \textbf{Descripción y Nivel de Acceso} \\ \hline
\textbf{Estudiantes y Egresados} & Son el motor de la plataforma. Ingresan al sistema para construir su Currículum Vitae Virtual (CVV), donde detallan su nivel de avance, conocimientos técnicos, disponibilidad horaria y pretensiones salariales. Tienen acceso para buscar, filtrar y postular con un solo clic a las ofertas que el sistema les sugiere según su porcentaje de compatibilidad. \\ \hline
\textbf{Empresas Externas} & Son las entidades que inyectan oportunidades al sistema. Tras pasar por una validación estricta de seguridad, acceden a un panel de control donde publican vacantes estructuradas, adjuntan documentación y administran el flujo de postulantes, visualizando en detalle los CVV de los candidatos recomendados. \\ \hline
\textbf{Áreas Internas UFT} & Unidades como laboratorios, centros de investigación o direcciones de la propia universidad. Operan en el sistema con la misma lógica que una empresa externa, publicando oportunidades de ayudantías, prácticas internas o trabajos exclusivos para la comunidad Finis Terrae. \\ \hline
\textbf{Administradores} & Personal encargado de la gobernanza transaccional de la plataforma. Tienen control total para auditar el registro de empresas (evitando ofertas informales), moderar el contenido de las vacantes publicadas, supervisar el correcto funcionamiento del motor de recomendaciones y extraer reportes de la base de datos. \\ \hline
\textbf{Visitantes} & Usuarios anónimos que llegan a la plataforma. Poseen un acceso limitado de solo lectura que les permite explorar el catálogo de ofertas públicas como un "gancho" para registrarse, pero el sistema les bloquea a\subsection{Ambiente Operacional de la Solución}%(cami)
=======
...
\subsubsection*{Infraestructura del sistema}
...
\begin{itemize}
    \item \textbf{Procesador:} CPU de al menos 4 núcleos físicos (idealmente 8 núcleos) con una frecuencia base de 2.5 GHz o superior, así se pueden atender solicitudes concurrentes sin degradación del tiempo de respuesta.
    \item \textbf{Memoria RAM:} Mínimo 16 GB de RAM para soportar la ejecución del servidor de aplicaciones, el servidor de base de datos y los procesos del módulo de recomendación de forma simultánea.
    \item \textbf{Almacenamiento:} Disco de estado sólido (SSD) con una capacidad mínima de 500 GB. Esto considerando el volumen de perfiles CVV, ofertas laborales, documentos adjuntos y respaldos periódicos de la base de datos. 
    \item \textbf{Conectividad:} Interfaz de red de al menos 1 Gbps, con conexión estable a internet y soporte para tráfico HTTPS cifrado.
    \item \textbf{Disponibilidad:} El servidor debe contar con una fuente de alimentación redundante o encontrarse respaldado por un sistema UPS (\textit{Uninterruptible Power Supply}) para minimizar interrupciones no planificadas y evitar perdidas de datos.
\end{itemize}
...
Finalmente, se considera la posibilidad de implementar el sistema bajo un esquema de computación en la nube (\textit{cloud computing}), utilizando proveedores como AWS, Google Cloud o Microsoft Azure. Esta alternativa presenta ventajas en términos de escalabilidad dinámica, alta disponibilidad y reducción de costos de mantención de hardware físico, permitiendo ajustar los recursos de cómputo y almacenamiento en función de la demanda real del sistema. En cualquiera de los dos escenarios —servidor físico o nube— la arquitectura de despliegue debe garantizar la separación lógica entre el servidor de aplicaciones, el servidor web y la base de datos, conforme a la arquitectura multicapa definida para el sistema.
